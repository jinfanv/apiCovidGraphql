...
module.exports = {
  ...
  addRestriction: async (root, { countryId, restrictionId }) => {
    ...
    try {
      ...
      restriction = await db
        ...
        .findOne({ _id: ObjectID(restrictionId) });
  ...
  editCountry: async (root, { _id, input }) => {
    ...
    try {
      ...
      Country = await db.collection("Country").findOne({ _id: ObjectID(_id) });
    } catch (error) {
      console.error(error);
    }
  ...
  editBed: async (root, { _id, input }) => {
    ...
    try {
      ...
      await db
        .collection("Beds")
        .updateOne({ _id: ObjectID(_id) }, { $set: input });
      Bed = await db.collection("Beds").findOne({ _id: ObjectID(_id) });
    } catch (error) {
      console.error(error);
    }
  ...
  deleteCountry: async (root, { _id }) => {
    ...
    try {
      db = await connectDb();
      await db.collection("Country").deleteOne({ _id: ObjectID(_id) });
    } catch (error) {
      console.error(error);
    }
  ...
  deleteRestriction: async (root, { _id }) => {
    let db;
